...
const PaginationWithoutApi = ({
    ...
    useEffect(() => {
        setShowData(rowData.slice(
            (currentPage - 1) * recordsPerPage,
            currentPage * recordsPerPage
        ))
        setRemainingData(rowData.slice(
            (currentPage - 1) * recordsPerPage,
            currentPage * recordsPerPage
        ))
    }, [currentPage])
    ...
    return (
        <div className="border text-sm border-[#BABFC7] mt-[-2px] gap-10 px-4 py-2 rounded-br-sm rounded-bl-sm flex items-center justify-end">
            <p>
                ...
                {showData.length}<span className="font-bold"> out of</span> {totalRecords}
            </p>
...
export default PaginationWithoutApi;
